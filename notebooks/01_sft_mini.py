...
# Base Qwen2.5-bnb-4bit weights use a tokenizer without chat_template — copy from Instruct
if not getattr(tokenizer, "chat_template", None):
    ...
    _ref_name = _INSTRUCT.get(BASE_MODEL, "Qwen/Qwen2.5-3B-Instruct")
    _ref_tok = AutoTokenizer.from_pretrained(_ref_name)
    tokenizer.chat_template = _ref_tok.chat_template
    print(f"Copied chat_template from {_ref_name}")
...
#
# `bkai-foundation-models/vi-alpaca` (~50k VN Alpaca rows; instruction/input/output).
# Legacy default `5CD-AI/Vietnamese-alpaca-cleaned` was removed from the Hub.
# Lab 21 uses a 1k slice for the demo run; we match that for comparable reward gaps.
...
ds = None
for name in dict.fromkeys(_SFT_FALLBACKS):  # preserve order, dedupe
    if not name:
        continue
    try:
        ds = load_dataset(name, split=f"train[:{SFT_SLICE}]")
...
if ds is None:
    raise RuntimeError(
        "No SFT dataset could be loaded. Set SFT_DATASET env to a public VN Alpaca dataset."
    )
...
from _lab22_colab_utils import patch_dataset_map_no_mp

patch_dataset_map_no_mp()

ds_formatted = ds.map(
    format_alpaca_to_chat, remove_columns=ds.column_names, num_proc=None
)
...
# Colab/Jupyter: multiprocessing during tokenize causes PicklingError with Unsloth
os.environ.setdefault("TOKENIZERS_PARALLELISM", "false")

from trl import SFTTrainer, SFTConfig

# ~4 warmup steps for 1k samples / effective batch 8 / 1 epoch
_warmup_steps = max(1, int(1000 / (PER_DEVICE_BATCH * GRAD_ACCUM) * NUM_EPOCHS * 0.03))
...
sft_config = SFTConfig(
    ...
    dataset_num_proc=None,     # None = no subprocesses (1 still pickles on Colab)
    report_to="none",
)

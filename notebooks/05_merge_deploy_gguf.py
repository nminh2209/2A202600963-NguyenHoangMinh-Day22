...
# Stack SFT-mini → DPO adapters (both needed for aligned export)
...
# %% [markdown]
# > **Note:** The DPO adapter stacks on top of SFT. For **bonus GGUF** we export
# > directly with ``save_pretrained_gguf`` — Unsloth merges adapters internally.
# >
# > **Do not** use ``merge_and_unload()`` + ``save_pretrained()`` on Colab 4-bit:
# > newer ``transformers`` raises ``NotImplementedError`` in ``reverse_op``.
# > Merged FP16 under ``adapters/merged-fp16/`` is optional (vLLM only) and skipped here.

# %% [markdown]
# ## 2. Export GGUF Q4_K_M (bonus deliverable)
#
# Q4_K_M is the sweet spot: ~4× compression vs FP16, minimal quality loss.
# Unsloth wraps llama.cpp — first run downloads + compiles llama.cpp (~3 min)
# then quantizes (~30 s). **Grading only needs `gguf/*.gguf`** — not merged-fp16.
...
# %% [markdown]
# ### 2a. Optional — additional quantization tiers (for the +3 rigor add-on)
...
# %% [markdown]
# ## 4. Optional — vLLM serving (BigGPU only)
#

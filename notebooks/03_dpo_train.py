...
if not getattr(tokenizer, "chat_template", None):
    ...
    _ref = AutoTokenizer.from_pretrained(_INSTRUCT.get(BASE_MODEL, "Qwen/Qwen2.5-3B-Instruct"))
    tokenizer.chat_template = _ref.chat_template
    print(f"Copied chat_template from {_INSTRUCT.get(BASE_MODEL)}")
...
# T4 (sm_75): xformers FA backward needs Ampere+ — use SDPA before any train forward
...
dpo_config = DPOConfig(
    ...
    warmup_steps=max(1, int(1000 / (PER_DEVICE_BATCH * GRAD_ACCUM) * EPOCHS * 0.1)),
...
print(f"dataset_num_proc={dpo_config.dataset_num_proc!r}  (None = no multiprocessing)")
...
# ## 4. Train
#
# > **Colab:** use this single cell only (creates `trainer` + runs `train()`).
# > If you see `AcceleratorState` / `mixed_precision` / `is_fsdp2` errors:
# > **Runtime → Restart session**, then re-run **C → D → NB3** (model load through here).
...
patch_dataset_map_no_mp()

# --- accelerate: version + clean singleton (Colab mid-session pip upgrades break this) ---
import accelerate
import torch
...
def _acc_major_minor(ver: str) -> tuple[int, int]:
    nums = []
...
if _acc_major_minor(accelerate.__version__) < (1, 10):
    print(f"Upgrading accelerate {accelerate.__version__} → >= 1.10 ...")
    subprocess.check_call(
        [sys.executable, "-m", "pip", "install", "-q", "-U", "accelerate>=1.10,<2.0"]
    )
    raise RuntimeError(
        "accelerate upgraded. Runtime → Restart session, then re-run sections C–D and this cell."
    )

# Wipe poisoned singleton from mid-session pip upgrades
...
# PartialState + Accelerator() — do NOT call AcceleratorState() directly (raises ValueError)
PartialState()
_mp = "bf16" if torch.cuda.is_bf16_supported() else "fp16"
_warmup_acc = Accelerator(mixed_precision=_mp)
del _warmup_acc
gc.collect()

# Drop any trainer built before state reset (stale accelerator reference)
...
# Re-apply SDPA patch (train cell may run after mid-session xformers reinstall)
force_sdpa_if_turing()

print(f"accelerate {accelerate.__version__} · DPO train starting (~15–25 min on T4)...")
